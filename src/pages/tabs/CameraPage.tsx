...
const CameraPage: React.FC = () => {
    ...
    const restartCamera = async () => {
        ...
        try {
            ...
            console.error(
                "Erreur lors de la réinitialisation de la caméra:",
                error
            );
        } finally {
            if (isMounted.current) {
    ...
    const sendToSelectedFriends = () => {
        ...
        setTimeout(() => {
            ...
            setToastMessage(
                `Envoyé à ${friendCount} ami${friendCount > 1 ? "s" : ""}!`
            );
            setShowToast(true);
        }, 1500);
    };
            ...
            <IonContent fullscreen className="camera-content">
                ...
                {cameraActive && !photo && !isRecording && !showFriendsList && (
                    <div className="camera-controls">
                        <div
                            className="control-button flash"
                            onClick={toggleFlash}
                        >
                            <IonIcon
                                icon={flashOutline}
                ...
                {showFriendsList && (
                    <div
                        className={`friends-selection-container ${
                            selectedFriends.length > 0 ? "selection-mode" : ""
                        }`}
                    >
                        <div className="friends-header">
                            ...
                            <div className="selected-count">
                                {selectedFriends.length > 0 && (
                                    <>
                                        {selectedFriends.length} sélectionné
                        ...
                        {selectedFriends.length === 0 && (
                            <div className="instruction-message">
                                <p>
                                    Appuyez sur un contact pour le sélectionner
                                </p>
                            </div>
                        )}
